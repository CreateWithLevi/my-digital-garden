---
title: AI 協作學習實驗室
date: 2026-07-05
tags:
  - AI
  - 學習
  - PKM
  - Lab
aliases:
  - AI Collaboration Learning Lab
description: 一組用來練習把輸入轉成具體成果，並把具體成果包裝成影響力實驗的互動 HTML lessons。
---

## AI 協作學習實驗室

這是一個 70 分版本的公開 learning lab。

它不是完整課程，而是我最近在練習的一組小型互動 lessons：如何把 input 轉成具體成果，再把具體成果放到合適的平台，測試反饋與影響力。

目前先放在 Digital Garden，作為可以慢慢修改、補英文版、或未來整理成 GitHub repo / resource page 的版本。

## Lessons

| Lesson | 你會練習什麼 | 打開 |
|---|---|---|
| Lesson 0001 — 具體成果迴圈 | 把模糊 input 轉成下一個最小具體成果 | <a href="https://createwithlevi.com/lab/lessons/0001-artifact-loop.html">Open</a> |
| Lesson 0002 — 先定義輸出，再和 AI 協作 | 用 output-first prompt 讓 AI 產出可修改版本 | <a href="https://createwithlevi.com/lab/lessons/0002-output-first-ai-collaboration.html">Open</a> |
| Lesson 0003 — 把具體成果包裝成影響力實驗 | 選平台、分發方式、觀察反饋與產品想像 | <a href="https://createwithlevi.com/lab/lessons/0003-package-output-for-impact.html">Open</a> |

## References

- <a href="https://createwithlevi.com/lab/reference/artifact-loop.html">具體成果迴圈 reference</a>
- <a href="https://createwithlevi.com/lab/reference/output-impact-map.html">具體成果影響力地圖 reference</a>

## Recommended path

1. 先讀：[[zh/input-to-concrete-output|從輸入到具體成果，是一種可以訓練的能力]]
2. 再做 Lesson 0001 和 Lesson 0002。
3. 如果你已經有一個成果想公開，做 Lesson 0003。
4. 最後回頭修改自己的筆記、文章、專案或下一步行動。

## Preview

<iframe src="https://createwithlevi.com/lab/lessons/0003-package-output-for-impact.html" style="width:100%; min-height:760px; border:1px solid var(--lightgray); border-radius:12px; background:white;"></iframe>
